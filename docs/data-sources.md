...
| Radar site metadata (lat/lon/elevation/time zone) | Bundled `app/res/config/radar_sites.json`, copied unmodified from Supercell Wx's `scwx-qt/res/config/radar_sites.json` (MIT, compiled from public NOAA/NWS sources) | `nimbus::data::FindRadarSite` (`app/source/nimbus/data/radar_site_database.hpp`), Phase 1 slice 2 | Static/offline, no network access needed. Also the intended lookup for §4.7's beam-height site lat/lon/elevation - reuse this then, don't add a second site database. |
