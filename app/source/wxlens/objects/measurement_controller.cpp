...
public:
   ...
   /// Distance is complete at two vertices; further clicks must not extend it into a path.
...
QString MeasurementController::readout() const
{
   ...
   if (!p->points_.empty() && p->points_.front().snapKind == QStringLiteral("radar"))
   {
...
QVariantMap MeasurementController::snapPreview() const
{
   if (!p->hasCursor_ || p->cursor_.snapKind.isEmpty())
   {
      return {{QStringLiteral("snapped"), false}};
   }
   return {{QStringLiteral("snapped"), true},
           {QStringLiteral("latitude"), p->cursor_.latitude},
           {QStringLiteral("longitude"), p->cursor_.longitude},
           {QStringLiteral("kind"), p->cursor_.snapKind},
           {QStringLiteral("label"), p->cursor_.snapLabel}};
}

void MeasurementController::addPoint(double                 latitude,
                                     double                 longitude,
                                     panes::PaneController* pane,
                                     const QString&         snapKind,
                                     const QString&         snapLabel)
{
   ...
   p->points_.push_back({latitude, longitude, snapKind, snapLabel});
   p->hasCursor_ = false;
...
void MeasurementController::updateCursor(double         latitude,
                                         double         longitude,
                                         const QString& snapKind,
                                         const QString& snapLabel)
{
   ...
   p->cursor_    = {latitude, longitude, snapKind, snapLabel};
   p->hasCursor_ = true;
