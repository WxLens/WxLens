...
class MeasurementController : public QObject
{
   ...
   Q_PROPERTY(QVariantMap snapPreview READ snapPreview NOTIFY measurementChanged)

public:
   enum class Mode
   {
      None = 0,
      Distance, ///< click A, click B; radar-relative automatically when A snapped to a site
   ...
   [[nodiscard]] QVariantMap  snapPreview() const;

   void setMode(int mode);

   /**
    * Adds a committed vertex. Distance completes at two vertices; Path keeps accumulating.
    */
   Q_INVOKABLE void addPoint(double latitude, double longitude,
                             wxlens::panes::PaneController* pane,
                             const QString& snapKind = {}, const QString& snapLabel = {});
    ...
    *
    * Anything already in progress is discarded, since a new press means a new measurement.
    */
   Q_INVOKABLE void beginDrag(double latitude, double longitude,
                              wxlens::panes::PaneController* pane,
                              const QString& snapKind = {}, const QString& snapLabel = {});
    ...
    */
   Q_INVOKABLE int commit(wxlens::panes::PaneController* pane, int scopeKind);
